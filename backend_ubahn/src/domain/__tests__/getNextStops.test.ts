...
xdescribe("cyclic line", () => {
  ...
  it(`continues with starting station if end is reached when the length of station is more than the length of stations in the line`, () => {
    const nextStops = getNextStops(
      cyclicLine,
      Direction.Forward,
      7,
    ...
    expect(nextStops).toStrictEqual(["Station5", "Station1", "Station2", "Station3", "Station4", "Station5", "Station1"]);
  });

  it(`returns the next 3 stops if 3 is passed when the length of stations is less than the length of stations in the line`, () => {
    const nextStops = getNextStops(
      cyclicLine,
      Direction.Forward,
      3,
